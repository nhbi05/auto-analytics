...
import { Activity, BarChart3, Bot, CircleHelp, Database, HeartPulse, Menu, Send, X } from 'lucide-react'
...
type AskResult = {answer:string;sql:string;analysis_type:string;data:Row[];chart:{type:string;title:string;x_column:string;y_column:string;data:Row[]}}
...
function ResultChart({result}:{result:AskResult}){
  ...
  if(result.analysis_type==='projection'){
    ...
    content=<LineChart data={c.data} margin={{left:12,right:18}}>
      <CartesianGrid strokeDasharray="3 3" vertical={false}/>
      <XAxis dataKey="period" tickFormatter={label}/>
      <YAxis tickFormatter={value=>compactNumber.format(Number(value))} width={58}/>
      <Tooltip labelFormatter={label} formatter={(value,name)=>[number.format(Number(value)),name==='actual'?'Historical':'Projected']}/>
      <Line dataKey="actual" name="Historical" stroke="#285c78" strokeWidth={3} dot={false} connectNulls={false}/>
      <Line dataKey="projected" name="Projected" stroke="#21a179" strokeWidth={3} strokeDasharray="7 5" dot={{r:3}} connectNulls={false}/>
    </LineChart>
...
function DataTable({rows}:{rows:Row[]}){if(!rows.length)return null;const columns=Object.keys(rows[0]);const display=(value:Row[string])=>typeof value==='number'?number.format(value):String(value??'—');return <div className="table-wrap"><table><thead><tr>{columns.map(c=><th key={c}>{c.replaceAll('_',' ')}</th>)}</tr></thead><tbody>{rows.map((row,i)=><tr key={i}>{columns.map(c=><td key={c}>{display(row[c])}</td>)}</tr>)}</tbody></table></div>}

function AskDatabase({domain}:{domain:Domain}){
  const [questions,setQuestions]=useState<string[]>([]),[question,setQuestion]=useState(''),[result,setResult]=useState<AskResult|null>(null),[loading,setLoading]=useState(false),[error,setError]=useState('')
  ...
  return <><header className="page-heading"><span>{domain}</span><h1>Ask the DB</h1></header><section className="ask-panel"><label htmlFor="demo">Start with an example</label><select id="demo" value="" onChange={e=>setQuestion(e.target.value)}><option value="">Choose a demonstration question…</option>{questions.map(q=><option key={q}>{q}</option>)}</select><form onSubmit={submit}><textarea value={question} onChange={e=>setQuestion(e.target.value)} placeholder={domain==='benefits'?'How much has been paid successfully?':'Which network has the most members?'} rows={3}/><button disabled={loading||!question.trim()}>{loading?'Analysing…':<><Send size={18}/> Ask database</>}</button></form></section>{error&&<ErrorBox message={error}/>} {result&&<section className="results"><div className="answer"><Bot size={22}/><p>{result.answer}</p></div><ResultChart result={result}/><details><summary>Generated SQL</summary><pre>{result.sql}</pre></details><DataTable rows={result.data}/><p className="safety">SQL is validated and executed in a read-only transaction with a row limit.</p></section>}</>
}
